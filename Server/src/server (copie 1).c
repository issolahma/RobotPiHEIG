...
// home/maude/Documents/PRO/FORK/RobotPiHEIG/Server/src/key.pem
    if (SSL_CTX_use_PrivateKey_file(ctx, "/etc/pki/tls/private/key.pem", SSL_FILETYPE_PEM) <= 0 ) {
        ERR_print_errors_fp(stderr);
	exit(EXIT_FAILURE);
    }
}
...
int server() {
	...
	SSL_set_mode(ssl, SSL_MODE_AUTO_RETRY); // SSL_MODE_ENABLE_PARTIAL_WRITE SSL_MODE_AUTO_RETRY
